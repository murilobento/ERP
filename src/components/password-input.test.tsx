...
describe('PasswordInput', () => {
  it('renders the password input correctly', async () => {
    ...
    const showPasswordButton = getByRole('button', { name: /mostrar senha/i })
  ...
  it('toggles the password visibility when the show password button is clicked', async () => {
    ...
    const hidePasswordButton = getByRole('button', { name: /ocultar senha/i })
    ...
    await expect.element(passwordInput).toHaveAttribute('type', 'password')
    await expect
      .element(getByRole('button', { name: /mostrar senha/i }))
      .toBeInTheDocument()
  })
